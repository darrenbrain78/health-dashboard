...
const CONTEXT_PATH = (0, path_1.join)(__dirname, "health-context.json");
let healthContext = null;
function loadContext() {
    if (!(0, fs_1.existsSync)(CONTEXT_PATH)) {
        console.warn("⚠  health-context.json not found. Run: npx tsx scripts/build-health-context.ts");
        return null;
    }
    const raw = (0, fs_1.readFileSync)(CONTEXT_PATH, "utf-8");
    return JSON.parse(raw);
}
// ── Build system prompt from context ─────────────────────────────────────
function buildSystemPrompt(ctx, liveContext) {
    const parts = [];
...
// ── Discord dispatch ─────────────────────────────────────────────────────
async function dispatchToDiscord(webhookUrl, content, username = "Health Dashboard") {
    try {
...
function detectChannel(userMessage) {
    const lower = userMessage.toLowerCase();
    if (/\b(run|running|garmin|workout|training|exercise|strength|gym|conjugate|squat|deadlift|bench)\b/.test(lower))
        return "fitness";
    if (/\b(sleep|rem|hrv|readiness|recovery|nap|insomnia|melatonin|magnesium)\b/.test(lower))
        return "sleep";
    if (/\b(blood|lab|ferritin|cholesterol|vitamin|thyroid|testosterone|iron|liver|kidney)\b/.test(lower))
        return "health";
    return null;
}
// ── Express server ───────────────────────────────────────────────────────
...
app.post("/api/chat", async (req, res) => {
    ...
    try {
        const client = new sdk_1.default({ apiKey: ANTHROPIC_API_KEY });
        ...
        const sources = [];
        if (healthContext?.topics["fitness-oura-analysis"])
            sources.push("oura");
        if (healthContext?.garminCache)
            sources.push("garmin");
        if (healthContext?.topics["health-blood-tests"])
            sources.push("labs");
        if (liveContext && Object.keys(liveContext).length > 0)
            sources.push("live");
